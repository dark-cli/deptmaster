...
class _ContactPermissionsScreenState extends ConsumerState<ContactPermissionsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final perms = await Api.getContactGroupPermissions(widget.walletId);
      if (mounted) {
        setState(() {
          _permissions = perms;
          _loading = false;
  ...
  Future<void> _togglePermission(Map<String, dynamic> perm, String sourceGroup, String contactGroup, String actionName) async {
    final isDeny = perm['is_deny'] as bool? ?? false;

    try {
      final newEntries = _permissions.map((p) {
        if (p['source_group_id'] == sourceGroup && p['contact_group_id'] == contactGroup && p['action'] == actionName) {
          return {...p, 'is_deny': !isDeny};
        }
        return p;
      }).toList();

      await Api.setContactGroupPermissions(widget.walletId, newEntries);
      await _load();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _permissions.isEmpty
          ? Center(
              child: Text(
                'No contact permissions configured',
          ...
          : ListView(
              ...
              children: _permissions.map((perm) {
                ...
                final contactGroup = perm['contact_group_id'] as String? ?? '';
                final action = perm['action'] as String? ?? '';
                final isDeny = perm['is_deny'] as bool? ?? false;

                return ListTile(
                  title: Text('$sourceGroup → $contactGroup'),
                  ...
                  onTap: () => _togglePermission(perm, sourceGroup, contactGroup, action),
                );
              }).toList(),
            ),
    );
