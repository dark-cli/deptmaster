...
pub fn push_unsynced() -> Result<(), String> {
    ...
    // The "id" field being sent is the event_id generated by the client.
    // This is WRONG - the server generates event_id at insert time.
    // Should send "idempotency_key" instead (client-generated per action for dedup).
    // Also need to store returned event_id from server for future operations.
    let payload: Vec<String> = unsynced
        .iter()
        .map(|e| {
            let v = serde_json::json!({
                "id": e.id,  // TODO: Change to "idempotency_key" when fixing client architecture
