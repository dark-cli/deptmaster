...
pub fn get_member_permissions(wallet_id: String) -> Result<String, String> {
    api::get_member_permissions_api(&wallet_id).map_err(|e| e.to_string())
}

pub fn set_member_permissions(wallet_id: String, entries_json: String) -> Result<(), String> {
    api::set_member_permissions_api(&wallet_id, &entries_json)?;
    if let Ok(Some(current)) = database::storage::config_get("current_wallet_id") {
        if current == wallet_id {
            let _ = services::sync::clear_wallet_and_resync(&wallet_id);
        }
...
pub fn get_contact_group_permissions(wallet_id: String, contact_group_id: String) -> Result<String, String> {
    api::get_contact_group_permissions_api(&wallet_id, &contact_group_id).map_err(|e| e.to_string())
}

pub fn set_contact_group_permissions(wallet_id: String, contact_group_id: String, entries_json: String) -> Result<(), String> {
    api::set_contact_group_permissions_api(&wallet_id, &contact_group_id, &entries_json)?;
    if let Ok(Some(current)) = database::storage::config_get("current_wallet_id") {
        if current == wallet_id {
            let _ = services::sync::clear_wallet_and_resync(&wallet_id);
        }
