...
use client::{
    ...
    clear_wallet_data, create_contact, create_wallet_contact_group, create_wallet_invite_code,
    create_wallet_user_group, delete_contact, get_contacts, join_wallet_by_code,
    list_wallet_contact_group_members, list_wallet_contact_groups, list_wallet_user_group_members,
    list_wallet_user_groups, manual_sync, put_wallet_permission_matrix, set_current_wallet_id,
    update_contact,
};
...
fn permission_give_take_read_member_sees_then_loses_then_sees() {
    ...
    member_in_wallet
        .sync()
        .expect("member sync to pull contact");
    member_in_wallet
    ...
    assert!(
        create_res.is_err(),
        "member with read-only must not be allowed to create contact"
    );
    let create_err = create_res.unwrap_err();
    assert!(
        create_err.contains("403") || create_err.to_lowercase().contains("permission"),
        "expected 403 or permission error"
    );

    owner.activate().expect("owner");
    owner
        .run_commands(&["permission revoke-all"])
        .expect("revoke all");
    ...
    assert!(
        create_after_revoke.is_err(),
        "member with no permissions must not be allowed to create contact"
    );

    owner.activate().expect("owner");
    owner
        .run_commands(&["permission grant-read"])
    ...
    assert!(
        create_after_regrant.is_err(),
        "member with read-only re-grant must not be allowed to create contact"
    );
}
...
fn permission_read_revoke_clears_local_then_grant_restores_via_sync() {
    ...
    owner
        .run_commands(&["permission revoke-all"])
        .expect("revoke all (read removed)");
    member_in_wallet.sync().expect("member sync after revoke");
    std::thread::sleep(std::time::Duration::from_millis(200));
    member_in_wallet
...
fn permission_grant_create_then_member_can_create() {
    ...
    owner.activate().expect("owner");
    owner
        .run_commands(&["permission grant-full"])
        .expect("grant full to members");
    ...
    member_in_wallet
        .assert_commands(&[
            ...
            "transaction description 'Member Tx'",
        ])
...
fn setup_owner_and_member() -> (AppInstance, AppInstance, String) {
    ...
    member
        .login()
        .expect("login member so API calls as member are authenticated");
...
fn permission_limits_deny_overrides_allow() {
    ...
    .expect("add to Editors");
    add_wallet_user_group_member(wallet_id.clone(), ug_bad.clone(), member.username.clone())
        .expect("add to BadActors");
    std::thread::sleep(std::time::Duration::from_millis(300));

    let cg_all = get_contact_group_id(&wallet_id, "all_contacts").expect("all_contacts");
    set_permission_matrix_allowed_denied(
        &wallet_id,
        &ug_editors,
        &cg_all,
        vec!["contact:create".to_string(), "contact:read".to_string()],
        vec![],
    )
    .expect("set");
    ...
    assert!(
        res.is_ok(),
        "User should be able to create contact with Allow permission"
    );
    ...
    assert!(
        contacts.iter().any(|c| c["name"] == "Allowed Contact"),
        "Allowed Contact should be present"
    );
    assert!(
        !contacts.iter().any(|c| c["name"] == "Denied Contact"),
        "Denied Contact must not be present (server rejected push)"
    );
}
...
fn permission_limits_union_of_groups() {
    ...
    create_contact("Test Contact".to_string(), None, None, None, None, None)
    ...
    set_permission_matrix_allowed_denied(
        ...
        &ug_updaters,
        &cg_all,
        vec!["contact:update".to_string(), "contact:read".to_string()],
        vec![],
    )
    .expect("set");
    ...
    let contact = contacts
        .iter()
        .find(|c| c["name"] == "Test Contact")
        .expect("should see contact");
    ...
    assert!(
        res.is_ok(),
        "Should be able to update due to Updaters group"
    );
...
fn permission_limits_scoped_denial() {
    ...
    create_wallet_contact_group(wallet_id.clone(), "Personal".to_string())
    ...
    let c_work_json = create_contact("Work Contact".to_string(), None, None, None, None, None)
        .expect("create work c");
    let c_work: serde_json::Value = serde_json::from_str(&c_work_json).expect("parse");
    let c_work_id = c_work["id"].as_str().unwrap().to_string();
    add_wallet_contact_group_member(wallet_id.clone(), cg_work.clone(), c_work_id)
        .expect("add to work");

    let c_personal_json =
        create_contact("Personal Contact".to_string(), None, None, None, None, None)
            .expect("create personal c");
    ...
    add_wallet_contact_group_member(wallet_id.clone(), cg_personal.clone(), c_personal_id)
        .expect("add to personal");
    ...
    let contacts: Vec<serde_json::Value> = serde_json::from_str(&contacts_json).expect("parse");
    assert!(
        contacts.iter().any(|c| c["name"] == "Work Contact"),
        "Should see Work Contact"
    );
    assert!(
        !contacts.iter().any(|c| c["name"] == "Personal Contact"),
        "Should NOT see Personal Contact (Probation deny overrides Friends allow)"
    );
}
...
fn groups_user_group_create_list_add_member_list_members() {
    ...
    owner.activate().expect("owner");
    let create_res = create_wallet_user_group(wallet_id.clone(), "Editors".to_string())
        .expect("create user group");
    let created: serde_json::Value = serde_json::from_str(&create_res)
    ...
    assert_eq!(
        editors_found, editors_id,
        "list should return created group"
    );
    ...
    let members_json =
        list_wallet_user_group_members(wallet_id.clone(), editors_id).expect("list members");
    let members: Vec<serde_json::Value> = serde_json::from_str(&members_json)
        .map_err(|e| e.to_string())
        .expect("parse");
    assert!(
        !members.is_empty(),
        "Editors should have at least one member"
    );
    let has_member = members
        .iter()
        .any(|m| m.get("username").and_then(|v| v.as_str()) == Some(&member.username));
...
fn groups_contact_group_create_list_add_contact_list_members() {
    ...
    let create_res = create_wallet_contact_group(wallet_id.clone(), "VIP".to_string())
    ...
    let members_json =
        list_wallet_contact_group_members(wallet_id.clone(), vip_id).expect("list VIP members");
    let members: Vec<serde_json::Value> = serde_json::from_str(&members_json)
        .map_err(|e| e.to_string())
        .expect("parse");
    assert_eq!(members.len(), 1, "VIP should have one member");
    assert_eq!(
        members[0].get("contact_id").and_then(|v| v.as_str()),
...
fn groups_complex_member_sees_only_contacts_in_permitted_group() {
    ...
    let ug_res =
        create_wallet_user_group(wallet_id.clone(), "Viewers".to_string()).expect("create Viewers");
    let viewers_ug_id: String = serde_json::from_str::<serde_json::Value>(&ug_res).expect("parse")
        ["id"]
        .as_str()
        .expect("id")
        .to_string();
    ...
    .expect("add member to Viewers");

    let cg_res = create_wallet_contact_group(wallet_id.clone(), "Public".to_string())
        .expect("create Public");
    let public_cg_id: String = serde_json::from_str::<serde_json::Value>(&cg_res).expect("parse")
        ["id"]
        .as_str()
        .expect("id")
        .to_string();
    ...
    let contacts_json = get_contacts().expect("get_contacts");
    let alice_id = contact_id_by_name(&contacts_json, "Alice").expect("Alice id");
    add_wallet_contact_group_member(wallet_id.clone(), public_cg_id.clone(), alice_id)
        .expect("add Alice to Public");
    ...
    assert!(
        create_res.is_err(),
        "member with read-only on Public must not be allowed to create contact"
    );
}
...
fn groups_complex_two_user_groups_two_contact_groups_scoped_visibility() {
    ...
    .expect("parse")["id"]
        ...
        .expect("id")
        .to_string();
    let ug_group_a_only_id: String = serde_json::from_str::<serde_json::Value>(
        &create_wallet_user_group(wallet_id.clone(), "GroupAOnly".to_string()).expect("create"),
    )
    .expect("parse")["id"]
        .as_str()
        .expect("id")
    ...
    add_wallet_user_group_member(
        wallet_id.clone(),
        ug_viewers_id.clone(),
        member1.username.clone(),
    )
    .expect("member1 -> Viewers");
    add_wallet_user_group_member(
        wallet_id.clone(),
        ug_group_a_only_id.clone(),
        member2.username.clone(),
    )
    .expect("member2 -> GroupAOnly");

    let cg_a_id: String = serde_json::from_str::<serde_json::Value>(
        &create_wallet_contact_group(wallet_id.clone(), "GroupA".to_string()).expect("create"),
    )
    .expect("parse")["id"]
        .as_str()
        .expect("id")
        .to_string();
    let cg_b_id: String = serde_json::from_str::<serde_json::Value>(
        &create_wallet_contact_group(wallet_id.clone(), "GroupB".to_string()).expect("create"),
    )
    .expect("parse")["id"]
        .as_str()
        .expect("id")
    ...
    add_wallet_contact_group_member(wallet_id.clone(), cg_a_id.clone(), alice_id)
        .expect("Alice -> GroupA");
    add_wallet_contact_group_member(wallet_id.clone(), cg_b_id.clone(), bob_id)
        .expect("Bob -> GroupB");
    ...
    .expect("GroupAOnly x GroupA");

    let m1 = AppInstance::with_credentials(
        "m1",
        &server_url,
        member1.username.clone(),
        member1.password.clone(),
    );
    ...
    .expect("member1 sees Alice and Bob");
    assert!(
        m1.run_commands(&["contact create 'Denied' x"]).is_err(),
        "member1 read-only; create denied"
    );

    let m2 = AppInstance::with_credentials(
        "m2",
        &server_url,
        member2.username.clone(),
        member2.password.clone(),
    );
    ...
    std::thread::sleep(std::time::Duration::from_millis(200));
    m2.assert_commands(&["contacts count 1", "contact name 'Alice'"])
        .expect("member2 sees only Alice");
    assert!(
        m2.run_commands(&["contact create 'Denied' y"]).is_err(),
        "member2 read-only; create denied"
    );
}
...
fn groups_priority_custom_user_group_times_all_contacts_sees_all() {
    ...
    assert!(
        member_app
            .run_commands(&["contact create 'Denied' x"])
            .is_err(),
        "Viewers read-only; create denied"
    );
}
...
fn groups_priority_union_multiple_user_groups_see_union_of_contact_groups() {
    ...
    let contacts_json = get_contacts().expect("get_contacts");
    let alice_id = contact_id_by_name(&contacts_json, "Alice").expect("Alice id");
    add_wallet_contact_group_member(wallet_id.clone(), cg_vip_id.clone(), alice_id)
        .expect("Alice -> VIP");
    ...
    add_wallet_user_group_member(
        wallet_id.clone(),
        ug_viewers_id.clone(),
        member_viewers.username.clone(),
    )
    .expect("Viewers only");
    add_wallet_user_group_member(
        wallet_id.clone(),
        ug_vip_only_id.clone(),
        member_vip.username.clone(),
    )
    .expect("VIPOnly only");
    add_wallet_user_group_member(
        wallet_id.clone(),
        ug_viewers_id.clone(),
        member_both.username.clone(),
    )
    .expect("both 1");
    add_wallet_user_group_member(
        wallet_id.clone(),
    ...
    let app_viewers = AppInstance::with_credentials(
        "mv",
        &server_url,
        member_viewers.username.clone(),
        member_viewers.password.clone(),
    );
    ...
    app_viewers
        ...
        .expect("Viewers-only sees all");

    let app_vip = AppInstance::with_credentials(
        "mvip",
        &server_url,
        member_vip.username.clone(),
        member_vip.password.clone(),
    );
    ...
    app_vip
        .assert_commands(&["contacts count 1", "contact name 'Alice'"])
        .expect("VIPOnly sees only Alice");

    let app_both = AppInstance::with_credentials(
        "mboth",
        &server_url,
        member_both.username.clone(),
        member_both.password.clone(),
    );
    ...
    app_both
        ...
        .expect("Viewers+VIPOnly sees union = all");
}
...
fn groups_priority_different_ug_to_different_cg_scoped_and_union() {
    ...
    add_wallet_contact_group_member(wallet_id.clone(), cg_vip_id.clone(), alice_id)
        .expect("Alice -> VIP");
    add_wallet_contact_group_member(wallet_id.clone(), cg_staff_id.clone(), carol_id)
        .expect("Carol -> Staff");
    ...
    for (u, ug) in [
        ...
        add_wallet_user_group_member(wallet_id.clone(), ug.clone(), u.username.clone())
            .expect("ug");
    }

    let app_v = AppInstance::with_credentials(
        "m1",
        &server_url,
        member_viewers.username.clone(),
        member_viewers.password.clone(),
    ...
    app_v
        ...
        .expect("Viewers sees all");

    let app_e = AppInstance::with_credentials(
        "m2",
        &server_url,
        member_editors.username.clone(),
        member_editors.password.clone(),
    );
    ...
    std::thread::sleep(std::time::Duration::from_millis(200));
    app_e
        .assert_commands(&["contacts count 1", "contact name 'Carol'"])
        .expect("Editors sees only Staff (Carol)");
    ...
    app_vip
        .assert_commands(&["contacts count 1", "contact name 'Alice'"])
        .expect("VIPOnly sees only VIP (Alice)");
}
...
fn groups_two_apps_join_with_no_default_then_grant_via_contact_group() {
    ...
    assert_eq!(
        joined_wallet_id, wallet_id,
        "joined wallet is app1's wallet"
    );
    ...
    assert!(
        app2_in_wallet
            .run_commands(&["contact create 'Forbidden' x"])
            .is_err(),
        "app2 cannot create when no permissions"
    );

    app1.activate().expect("app1");
    let cg_shared_id: String = serde_json::from_str::<serde_json::Value>(
        &create_wallet_contact_group(wallet_id.clone(), "Shared".to_string())
            .expect("create contact group"),
    )
    .expect("parse")["id"]
        .as_str()
    ...
    let alice_id = contact_id_by_name(&contacts_json, "Alice").expect("Alice id");
    let bob_id = contact_id_by_name(&contacts_json, "Bob").expect("Bob id");
    add_wallet_contact_group_member(wallet_id.clone(), cg_shared_id.clone(), alice_id)
        .expect("Alice -> Shared");
    add_wallet_contact_group_member(wallet_id.clone(), cg_shared_id.clone(), bob_id)
        .expect("Bob -> Shared");
    set_matrix_actions(
        &wallet_id,
        &all_ug,
    ...
    app2_in_wallet
        .sync()
        .expect("app2 sync to pull with new permissions");
    std::thread::sleep(std::time::Duration::from_millis(200));
    app2_in_wallet
        .assert_commands(&[
            "contacts count 2",
...
fn permission_flow_default_read_then_none_then_group_read_write_three_apps() {
    ...
    let app2_instance = AppInstance::with_credentials(
        "app2",
        &server_url,
        app2.username.clone(),
        app2.password.clone(),
    ...
    assert!(
        app2_instance
            .run_commands(&["contact create 'No' x"])
            .is_err(),
    ...
    add_wallet_contact_group_member(wallet_id.clone(), cg_vip_id.clone(), alice_id)
        .expect("Alice -> VIP");
    add_wallet_contact_group_member(wallet_id.clone(), cg_vip_id.clone(), bob_id)
        .expect("Bob -> VIP");
    ...
    let alice = contacts
        .iter()
        .find(|c| c["name"].as_str() == Some("Alice"))
        .expect("Alice in VIP");
    ...
    let app3_instance = AppInstance::with_credentials(
        "app3",
        &server_url,
        app3.username.clone(),
        app3.password.clone(),
    ...
    assert!(
        app3_instance
            .run_commands(&["contact create 'No' y"])
            .is_err(),
        "app3 cannot create"
    );
