...
async fn main() -> anyhow::Result<()> {
    ...
    let scheduler = Arc::new(
        background::scheduler::BackgroundScheduler::new(db_pool.clone(), config.clone()).await?,
    );
    ...
    let wallet_protected_routes = Router::new()
        ...
        .route(
            "/api/wallets/:wallet_id/me/settings",
            get(handlers::get_my_wallet_settings).put(handlers::put_my_wallet_settings),
        )
        .route(
            "/api/wallets/:wallet_id/users",
            get(handlers::list_wallet_users).post(handlers::add_user_to_wallet),
        )
        .route(
            "/api/wallets/:wallet_id/users/search",
            get(handlers::search_wallet_users),
        )
        .route(
            "/api/wallets/:wallet_id/invite",
            axum::routing::post(handlers::create_wallet_invite),
        )
        .route(
            "/api/wallets/:wallet_id/users/:user_id",
            axum::routing::put(handlers::update_wallet_user)
                .delete(handlers::remove_user_from_wallet),
        )
        .route(
            "/api/wallets/:wallet_id/user-groups",
            get(handlers::list_user_groups).post(handlers::create_user_group),
        )
        .route(
            "/api/wallets/:wallet_id/user-groups/:group_id",
            axum::routing::put(handlers::update_user_group).delete(handlers::delete_user_group),
        )
        .route(
            "/api/wallets/:wallet_id/user-groups/:group_id/members",
            get(handlers::list_user_group_members).post(handlers::add_user_group_member),
        )
        .route(
            "/api/wallets/:wallet_id/user-groups/:group_id/members/:user_id",
            axum::routing::delete(handlers::remove_user_group_member),
        )
        .route(
            "/api/wallets/:wallet_id/contact-groups",
            get(handlers::list_contact_groups).post(handlers::create_contact_group),
        )
        .route(
            "/api/wallets/:wallet_id/contact-groups/:group_id",
            axum::routing::put(handlers::update_contact_group)
                .delete(handlers::delete_contact_group),
        )
        .route(
            "/api/wallets/:wallet_id/contact-groups/:group_id/members",
            get(handlers::list_contact_group_members).post(handlers::add_contact_group_member),
        )
        .route(
            "/api/wallets/:wallet_id/contact-groups/:group_id/members/:contact_id",
            axum::routing::delete(handlers::remove_contact_group_member),
        )
        .route(
            "/api/wallets/:wallet_id/permission-actions",
            get(handlers::list_permission_actions),
        )
        .route(
            "/api/wallets/:wallet_id/permission-matrix",
            get(handlers::get_permission_matrix).put(handlers::put_permission_matrix),
        )
        .route(
    ...
    let protected_api_routes = Router::new()
        ...
        .route(
            "/api/settings/:key",
            axum::routing::put(handlers::update_setting),
        )
        .route(
        ...
        .route(
            "/api/wallets",
            get(handlers::list_user_wallets).post(handlers::create_my_wallet),
        )
        .route(
            "/api/wallets/join",
    ...
    let admin_routes = Router::new()
        ...
        .route(
            ...
            get(handlers::get_latest_event_id),
        )
        .route(
            "/api/admin/events/backfill-transactions",
            post(handlers::backfill_transaction_events),
        )
        .route(
            "/api/admin/projections/status",
            get(handlers::get_projection_status),
        )
        .route("/api/admin/total-debt", get(handlers::get_total_debt))
        .route(
            "/api/admin/projections/rebuild",
            axum::routing::post(rebuild_projections),
        )
        ...
        .route(
            ...
            axum::routing::put(handlers::admin_change_password),
        )
        .route(
            "/api/admin/users/:id/login-logs",
            get(handlers::get_user_login_logs),
        )
        .route(
            "/api/admin/users/:id/backup",
            get(handlers::backup_user_data),
        )
        ...
        .route(
            "/api/admin/wallets/:id",
            axum::routing::delete(handlers::delete_wallet),
        )
        .route(
            "/api/admin/wallets/:id/users",
            get(handlers::list_wallet_users),
        )
        .route(
            "/api/admin/wallets/:id/users",
            post(handlers::add_user_to_wallet),
        )
        .route(
            "/api/admin/wallets/:id/users/:user_id",
            axum::routing::put(handlers::update_wallet_user),
        )
        .route(
            "/api/admin/wallets/:id/users/:user_id",
            axum::routing::delete(handlers::remove_user_from_wallet),
        )
        .layer(axum::middleware::from_fn_with_state(
            app_state.clone(),
            auth_middleware,
    ...
    if config.enable_tls {
        info!(
            "Server listening on http://{} (TLS handled by reverse proxy)",
            addr
        );
    } else {
