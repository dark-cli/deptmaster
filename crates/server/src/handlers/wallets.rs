...
pub async fn list_permission_actions(
    ...
    // Check wallet:info_read permission (owners bypass via hardcoded check)
    let can_read = crate::permissions::resolver::can_perform(
        &state.db_pool,
        &PermissionContext {
            wallet_id: wallet_uuid,
            user_id: auth_user.user_id,
    ...
    .await
    .map_err(|e| {
        tracing::error!("Error checking wallet:info_read permission: {:?}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"error": "Failed to check permissions"})),
        )
...
pub async fn get_wallet_permissions(
    ...
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
) -> Result<Json<Vec<WalletPermissionEntry>>, (StatusCode, Json<serde_json::Value>)> {
    let wallet_uuid = Uuid::parse_str(&wallet_id).map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": format!("Invalid wallet_id: {}", e)})),
    ...
    let perms: Vec<(Uuid, String, bool)> = sqlx::query_as(
        "SELECT user_group_id, action, is_deny FROM wallet_permission_matrix WHERE user_group_id IN (SELECT id FROM user_groups WHERE wallet_id = $1) ORDER BY user_group_id, action"
    )
    .bind(wallet_uuid)
    .fetch_all(&*state.db_pool)
    .await
    .map_err(|e| {
        tracing::error!("get_wallet_permissions: {:?}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let entries: Vec<WalletPermissionEntry> = perms
        .into_iter()
        .map(|(user_group_id, action, is_deny)| WalletPermissionEntry {
            user_group_id: user_group_id.to_string(),
            action,
            is_deny,
        })
...
/// Set wallet-level permissions (grant/revoke actions for groups)
...
// ============================================================================
// WALLET MEMBER PERMISSIONS (vector-based, scoped to target group)
// ============================================================================

#[derive(Serialize, Deserialize)]
pub struct MemberPermissionEntry {
...
pub async fn get_member_permissions(
    ...
    State(state): State<AppState>,
    Extension(auth_user): Extension<AuthUser>,
) -> Result<Json<Vec<MemberPermissionEntry>>, (StatusCode, Json<serde_json::Value>)> {
    let wallet_uuid = Uuid::parse_str(&wallet_id).map_err(|e| {
        (
            StatusCode::BAD_REQUEST,
            Json(serde_json::json!({"error": format!("Invalid wallet_id: {}", e)})),
    ...
    .await
    .map_err(|e| {
        tracing::error!("get_member_permissions: {:?}", e);
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(serde_json::json!({"error": "Failed to fetch permissions"})),
        )
    ...
    let entries: Vec<MemberPermissionEntry> = perms
        .into_iter()
        .map(|(source_id, target_id, action, is_deny)| MemberPermissionEntry {
            source_group_id: source_id.to_string(),
            target_group_id: target_id.to_string(),
            action,
            is_deny,
        })
        .collect();
...
/// Set vector-based member management permissions
