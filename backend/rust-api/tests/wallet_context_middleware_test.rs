...
async fn test_wallet_context_validation_inactive_wallet() {
    ...
    let wallet_is_active: bool = sqlx::query_scalar("SELECT is_active FROM wallets WHERE id = $1")
        .bind(wallet_id)
        .fetch_one(&pool)
        .await
        .unwrap();
...
async fn test_wallet_context_extraction_from_query() {
    ...
    assert!(
        query_wallet_id.is_some(),
        "Wallet ID should be extractable from query"
    );
...
async fn test_wallet_context_extraction_from_header() {
    ...
    headers.insert(
        "X-Wallet-Id",
        HeaderValue::from_str(&wallet_id.to_string()).unwrap(),
    );
    ...
    assert_eq!(
        header_wallet_id,
        Some(wallet_id),
        "Wallet ID should be extractable from header"
    );
}
