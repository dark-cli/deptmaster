...
prepare_rust_bridge_for_flutter() {
    local platform="$1"
    local crate_dir="$ROOT_DIR/crates/sdk"
    ...
    if [ "$platform" = "android" ]; then
        ...
        local so_count
        so_count=$(find "$jni_libs_dir" -name "libsdk.so" 2>/dev/null | wc -l)
        if [ "${so_count:-0}" -eq 0 ]; then
            print_error "No libsdk.so found under $jni_libs_dir. cargo ndk may have written to a different path."
            exit 1
        fi
        ...
        if (cd "$crate_dir" && cargo build --release 2>&1); then
            ...
            if [ -f "$crate_dir/target/debug/libsdk.so" ]; then
                mkdir -p "$crate_dir/target/release"
                cp -f "$crate_dir/target/debug/libsdk.so" "$crate_dir/target/release/" 2>/dev/null || true
            fi
        fi
...
cmd_run_flutter_app() {
    ...
    if [ "$platform" = "android" ]; then
        ...
        # Rust FFI: loader looks for libsdk.so in target/release
        local rust_lib_dir="$ROOT_DIR/crates/sdk/target/release"
        if [ -f "$rust_lib_dir/libsdk.so" ]; then
            export LD_LIBRARY_PATH="$rust_lib_dir${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
        else
            echo -e "${YELLOW}⚠ Rust lib not found at $rust_lib_dir - run: cd crates/sdk && cargo build --release (or: cp target/debug/libsdk.so target/release/)${NC}" >&2
        fi
        # Clean app data if --clear-app-data flag is set
        if [ "$CLEAR_APP_DATA" = true ]; then
